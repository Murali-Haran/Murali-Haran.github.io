...
plot(density(babies$bwt[babies$smoke=="Never"], na.rm=TRUE), col="blue", main="Birthweight according to smoking status", xlab="Baby birthweight in oz")# | babies$smoke=="Current"]))
...
babiesTab=table(babies$smoke, babies$ed) ## one way of tabulating
babiesTab2=table(babies$ed, babies$smoke) ## another way of tabulating!
...
barplot(as.matrix(babiesTab2),col=colEd, main="Smoking behavior across education levels") #, ylab="Counts of smokers in each category of ed")
...
barplot(as.matrix(babiesTab2),col=colEd, main="Smoking behavior across education levels",beside=TRUE) #, ylab="Counts of smokers in each category of ed")
##legend("topleft", c("First","Second","Third","Fourth","Fifth"), cex=1.3, bty="n", fill=colEd)
legend("topleft", c("Never","Current","Until","Once"), cex=1.3, bty="n", fill=colEd)
## Make this a better plot, adjust for the fact that different educ. categories have different numbers?
## Look at proportions
babiesProp=babiesTab2
babiesProp[,1]=babiesTab2[,1]/sum(babiesTab2[,1])
babiesProp[,2]=babiesTab2[,2]/sum(babiesTab2[,2])
babiesProp[,3]=babiesTab2[,3]/sum(babiesTab2[,3])
babiesProp[,4]=babiesTab2[,4]/sum(babiesTab2[,4])
##babiesProp[,5]=babiesTab2[,5]/sum(babiesTab2[,5])
...
names(babiesProp[,1]) # this is how to access names (more on this later under lists)
names(babiesProp[1,]) # this is how to access names (more on this later under lists)

## interaction plots FIX
interaction.plot(names(babiesProp[1,]), babiesProp[,1], type="l",xlab="smoke", ylab="prop", trace.label="prop")
##interaction.plot(babies$year,ga$sex,log(ga$rate),type="l",xlab="year",ylab="log(rate)",trace.label="sex")
