## To generate values from a Beta(\alpha,\beta) density, use the command {\tt rbeta}.
## To find out how to use {\tt R} commands, use `help', for example: 
help(rbeta)  # if you are within a unix/command line view, you can hit "q" to quit help
## generate 1000 Beta(4,9) r.v.s, store in mysim, a vector of 100 values.
mysim = rbeta(1000, 4, 9) 

## Similarly, you can generate 5000 random variates from a Poisson(25) distribution
mysim = rpois(5000, 25)

## To compute the mean and variance of an R vector, say myvec, you would use the commands: 
mean(mysim) 
var(mysim)

## You can initialize a vector in {\tt R} using `rep'
foo = rep(NA, 1000) # this creates a vector of length 100 with all ``NA''s (``missing values'')
...
## You can store the commands you want to run in a file, say foo.R.
## To run your commands, simply type in:
source("foo.R") # making sure that foo.R is in the same directory that you are running R from
