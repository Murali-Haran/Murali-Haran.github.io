...
\notes{Answer:  D.  This is an example that is worked out in Section 3.2.}
\end{frame}
...
{binomial}{Poisson}{uniform}{hypergeometric}
\notes{Answer:  A.  This is an example that is worked out in Section 3.2.}
\end{frame}
...
{binomial}{Poisson}{uniform}{hypergeometric}
\notes{Answer:  C.  To do this, we found the joint density of $X_1$ and $X_1+X_2$, and 
noticed that it did not depend on $X_1$.  So the conditional density of $X_1$ must be
uniform on its range.}
\end{frame}
...
{$\frac1{1-p}$}
{$\frac1{(1-p)^2}$}
\notes{Answer:  A.  We will derive this as a special case of a more complicated example 
in Section 3.4 next Wednesday.}
\end{frame}
